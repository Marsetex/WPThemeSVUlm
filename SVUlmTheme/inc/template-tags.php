...
function wp_bootstrap_starter_posted_on() {
	...
	$posted_on = sprintf(
		esc_html_x( 'Geschrieben am %s', 'post date', 'wp-bootstrap-starter' ),
		'<a href="' . esc_url( get_permalink() ) . '" rel="bookmark">' . $time_string . '</a>'
	);

	$byline = sprintf(
		esc_html_x( 'Von %s', 'post author', 'wp-bootstrap-starter' ),
...
function wp_bootstrap_starter_entry_footer() {
	...
	if ( 'post' === get_post_type() ) {
		...
		if ( $categories_list && wp_bootstrap_starter_categorized_blog() ) {
			printf( '<span class="cat-links">' . esc_html__( 'Tags: %1$s', 'wp-bootstrap-starter' ) . '</span>', $categories_list ); // WPCS: XSS OK.
		}
